...
const getMedia = async () => {
  try {
    const mediaArray = await getDataByProperty(
 ...
 */
const getPhotographerDetailsById = async (id) => {
  // return : object
  try {
    const photographerDetails = await getDataById(
      "../data/photographers.json",
      "photographers",
      id
    );
...
const getMediaById = async (id) => {
  try {
    ...
    const mediaById = mediaArray.media.filter(
      (obj) => obj.photographerId === JSON.parse(id)
    );
...
/**
 * description : display photographer media
 * @param {array} medias
 */
...
/**
 * description : display photographer details
 * @param {object} photographerDetails
 */
...
const displayPhotographerTotalLikesAndPrice = async (medias, photographer) => {
  ...
  const TotalLikesAndPriceDOM =
    TotalLikesAndPriceModel.getTotalLikesAndPriceDOM(totalLikes);
...
const initMediaLightbox = async (event) => {
  ...
  let indexLightbox = mediasArray.findIndex(
    (obj) => obj.id === parseInt(event.target.dataset.id)
  );
  index = indexLightbox;
